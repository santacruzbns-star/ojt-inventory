<x-app-layout>
    ...
    <div class="dashboard-wrapper py-6 bg-gray-50 min-h-screen">
        ...
        <script>
            const ctxBar = document.getElementById('nutritionStatusBarChart').getContext('2d');



            let nutritionChart = new Chart(ctxBar, {
                type: 'bar',
                data: chartData,
                options: {
                    responsive: true,
            ...
            document.querySelectorAll('input[name="gender"]').forEach(box => box.addEventListener('change', updateChart));

            function updateChart() {
                const selectedMonth = document.getElementById('monthFilter').value;
                const genders = Array.from(document.querySelectorAll('input[name="gender"]:checked')).map(e => e.value);
                ...
                axios.get('/dashboard', {
                    ...
                    .then(response => {
                        ...
                        // Update bar chart
                        nutritionChart.data.datasets[0].data = data.nutritionCounts;
                        nutritionChart.update();

                        // Update Top Zones (This will now receive ALL zones, regardless of ranking)
                        const topZonesList = document.querySelector('.top-zones-section .barangay-list');
                        topZonesList.innerHTML = `
                <li class="barangay-item font-bold" style="border-bottom: 2px solid #374151;">
                    <span>Zone</span>
